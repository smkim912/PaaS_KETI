...
enclib = cdll.LoadLibrary('./libdualenc.so')
enclib.aes_des_init()
...
def signal_handler(signal, frame):
	...
	while conn_cnt > 0:
		...
		enclib.aes_des_encrypt(UPDATECOMM, encUpdateComm, len(UPDATECOMM))
#		conn_list[conn_cnt-1].send(encUpdateComm)	//update comm error 170530 smkim
		conn = conn_list.pop()
		conn_cnt -= 1
		conn.close()
	time.sleep(1)
...
def decrypt_data(data):
	dec_data = create_string_buffer('\00'*2048)
	enclib.aes_des_decrypt.argtypes = [c_char_p, c_char_p, c_int]
	outlen = enclib.aes_des_decrypt(data, dec_data, len(data))

	ret = dec_data.raw[:outlen]
	return ret
...
#			print 'classifyBuff(): Multiple packet.'+' (real)'+str(len(buff))+' (header)'+str(h_len)
		...
		if equip == 'meter\x00\x00\x00':
			...
			print 'classifyBuff(): Unknown equipment -> ' + chunk.encode("hex")
...
#			print "data  > " + data
#			print "recv  > " + "".join("%02x" % ord(c) for c in data)
#			buff += data
			decdata = decrypt_data(data)
			print '<<<ori-data>>>\n' + data
			print '\n<<<dec-data>>>\n ' + decdata
			classifyBuff(decdata)
			time.sleep(0.1)
	conn.close()
